const findMaxFish=(grid)=>{
    ...
    function dfs(row,col,visited){
        ...
        sum+=dfs(row+1,col,visited)
        sum+=dfs(row-1,col,visited)
        sum+=dfs(row,col+1,visited)
        sum+=dfs(row,col-1,visited)
...
const findMaxFish2= (g) => {
    let areas = getAllAreasCoordinates(deepCopy2DArray(g)), res = 0;
...
const getAllAreasCoordinates = (g) => {
    const forbid = 0, floodFillMakeConnected = '*'; // forbid is land cell
    ...
    for (let i = 0; i < n; i++) {
        for (let j = 0; j < m; j++) {
            if (g[i][j] != forbid) {
                ...
                if (area.length == 0 && g[i][j] != floodFillMakeConnected) area.push([i, j]);
                res.push(area);
            }
        }
    }
...
console.log(findMaxFish([[4,5,5],[0,10,0]]))